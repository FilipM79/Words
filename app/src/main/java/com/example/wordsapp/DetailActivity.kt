...
//    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) { // replaced by android:name in xml
            val frTransaction = supportFragmentManager.beginTransaction()
            val letter = intent?.extras?.getString(LETTER).toString()
            frTransaction.replace(R.id.fragment_container, WordListFragment.newInstance(letter))
            frTransaction.commitNow()
        }
//
